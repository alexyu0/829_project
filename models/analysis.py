...
def mean_max_min(code, d):
    aggregates = []
    for i in model_constants.ge_ref_keys:
        latencies = [v[int(i)-1][1] for v in d.values()]
        bws = [v[int(i)-1][2] for v in d.values()]
        losses = [v[int(i)-1][3] for v in d.values()]
        # print(latencies)
...
def normalized_results(code):
    for latency in model_constants.LATENCY_VALS:
        ...
        for argset_key in model_constants.ge_ref_keys:
            ...
            with open(csv_file_path, "r") as f:
                ...
                for row in f_reader:
                    ...
                    if code == "RQ":
                        result_dict[" ".join(row[0:2])].append((row, 
                            float(row[4]),
                            float(row[5]),
                            float(row[6])))
                    else:
        ...
        # normalize
        aggregates = mean_max_min(code, result_dict)
...
def graph():
    for latency in model_constants.LATENCY_VALS:
        for metric in model_constants.METRICS:
            ...
            plt.plot(x_vals, rq_y_vals, linestyle='--', marker='o', color='b', label="RaptorQ")
            plt.plot(x_vals, midas_y_vals, linestyle='--', marker='o', color='r', label="MiDAS")
            ax.legend()
